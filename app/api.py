...
class BriefResponse(BaseModel):
    ...
    brief: Optional[FinalBrief] = Field(
        None, description="The completed research brief"
    )
    error: Optional[str] = Field(None, description="Error message if request failed")
    processing_time: Optional[float] = Field(
        None, description="Time taken to generate brief in seconds"
    )
    created_at: datetime = Field(
        default_factory=datetime.now, description="When this brief was created"
    )
...
async def root():
    ...
    """
    return HealthResponse(status="online", timestamp=datetime.now(), version="1.0.0")
...
async def get_active_requests():
    ...
    """
    return {"active_count": len(active_requests), "requests": active_requests}
...
async def get_performance_metrics():
    ...
    try:
        ...
        try:
            from future_implementation.langsmith_integration import (
                token_tracker,
                performance_monitor,
            )
...
if __name__ == "__main__":
    ...
    uvicorn.run(
        ...
        reload_excludes=[
            ...
            "__pycache__",
            "*.log",
        ],  # WHY: Don't reload for temporary files
    )
